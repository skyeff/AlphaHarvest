...
TIMEOUT,THREADS,CHUNK_SIZE=20,5,524288
RETRIES=Retry(total=3,backoff_factor=.3,status_forcelist=[429,500,502,503,504],allowed_methods=["GET"],raise_on_status=False)
MIN_SIZE,MIN_CHARS=10000,50
...
def DownloadPDF(s,u,p):
    try:
        with s.get(u,stream=True,timeout=TIMEOUT)as r:
            if r.status_code!=200:return False,f"http_{r.status_code}"
            with open(p,"wb")as f:
                for chunk in r.iter_content(CHUNK_SIZE):
                    if chunk:f.write(chunk);time.sleep(.001)
        return True,None
...
def SearchCrossRef(s,q,y):
    ...
    while cursor:
        try:
            ...
            if r.status_code!=200:print(f"\r  [X] Year {y}: HTTP {r.status_code}{' '*20}",flush=True);break
            ...
            time.sleep(.05)
        except requests.Timeout:print(f"\r  [X] Year {y}: timeout{' '*20}",flush=True);break
        except Exception as e:print(f"\r  [X] Year {y}: {type(e).__name__}{' '*20}",flush=True);break
    print(f"\r  [OK] Year {y}: {len(items)} harvested{' '*20}",flush=True)
    return items

def QueryUnpaywall(s,d,e):
    try:
        r=s.get(f"https://api.unpaywall.org/v2/{urllib.parse.quote(d)}",params={"email":e},timeout=TIMEOUT)
        return r.json()if r.status_code==200 else None
    except:return None

def ChoosePDFUnpaywall(d):
    if not d:return None,None
    candidates=filter(None,[d.get("best_oa_location")]+d.get("oa_locations",[]))
    for loc in candidates:
        for key in("url_for_pdf","url"):
            url=loc.get(key,"")
            if url and url.lower().endswith(".pdf"):
                return url,loc.get("version")
    return None,None

def QuerySemanticScholar(s,d):
    try:
        r=s.get(f"https://api.semanticscholar.org/graph/v1/paper/DOI:{urllib.parse.quote(d)}",params={"fields":"openAccessPdf"},timeout=TIMEOUT)
        if r.status_code==200:
            pdf=r.json().get("openAccessPdf",{}).get("url","")
            return pdf if pdf else None
    except:return None

def QueryOpenAlex(s,d):
    try:
        r=s.get(f"https://api.openalex.org/works/https://doi.org/{urllib.parse.quote(d)}",timeout=TIMEOUT)
        if r.status_code==200:
            pdf=r.json().get("open_access",{}).get("oa_url","")
            return pdf if pdf else None
    except:return None

def QueryCoreAC(s,d):
    try:
        r=s.get(f"https://core.ac.uk/api-v2/articles/get/doi/{urllib.parse.quote(d)}",timeout=TIMEOUT)
        if r.status_code==200:
            pdf=r.json().get("data",{}).get("downloadUrl","")
...
def process_item(it,s,a,filt):
    ...
    for fn,src in[(lambda:ChoosePDFUnpaywall(QueryUnpaywall(s,doi,a.email)),"Unpaywall"),
                  ...
                  (lambda:(QueryCoreAC(s,doi),None),"CORE")]:
        try:
    ...
    if not rec.get("pdf_url"):
        try:
            link=next((l.get("URL")for l in it.get("link",[])if l.get("content-type")=="application/pdf"),None)
            if link:rec.update({"pdf_url":link,"source":"CrossRef-Direct","version":"unknown"})
        except:pass
